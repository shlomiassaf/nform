 ...
 * returns a tuple [immediate prop name, rest of the path]
 */
 ...
 *   - exposing rendering records for a model (the records are used to render form elements)
 */
export class NForm<T = any> implements PropNotifyHandler {
  form: FormGroup;

  /**
   * When set, will proxy all incoming property changes from NFormRecordRef to this handler.
   */
  ...
  constructor(protected modelFormService: import('../services').NFormFactoryService) {}

  onPropChange(ri: NFormRecordRef, changes: PropChanges<NFormRecordRef>): void {} // tslint:disable-line

  /**
   * Retrieves a child control given the control's name or path.
   *
   * > This is method is just sugar for `nFormInstance.form.get(path)`
   *
   ...
   *
   * > This is method is just sugar for `nFormInstance.form.get(path).value`
   *
  ...
  getValueModel(item: NFormRecordRef, control: AbstractControl): any | null;
  getValueModel(path: Array<string | number> | string | NFormRecordRef, control?: AbstractControl): any | null {
    const pathArr =
      path instanceof NFormRecordRef
   ...
   *
   * > This is method is just sugar for `nFormInstance.form.get(path).setValue(value)`
   *
   * @param path The path, relative to the root form. Accepts the same types as `AbstractControl.get`.
   * See {@link https://angular.io/api/forms/AbstractControl#get}
  ...
  createChildForm<Z = any>(path: Array<string | number> | string,  model?: Z, formGroup?: FormGroup): NForm<Z> {
    const pathArr = normalizeFormPath<T>(path);
    const formProp = this.getFormProp(pathArr);
  ...
  createControl(path: string[] | string, value?: any, tryCreateNew?: boolean): FormGroup | FormControl {
    return NgFormsSerializeMapper.createControl(this.type, normalizeFormPath<T>(path), value, tryCreateNew);
  }
  ...
  appendControl(path: Array<string | number> | string, value?: any, tryCreateNew?: boolean): FormGroup | FormControl {
    ...
    if (formArray instanceof FormArray) {
      ...
      const ctrl = NgFormsSerializeMapper.createControl(this.type, <any>pathArr, value, tryCreateNew);
      formArray.push(ctrl);
      return ctrl;
    } else {
   ...
   * @returns
   */
  trackBy(idx: number, item: NFormRecordRef): any {
    return item.hash;
  }
   ...
   * @internal
   */
  bindRenderingData(controlRenderer: NFormControlTemplateContext, renderData: NFormRecordRef): void {
    controlRenderer.nForm = this;
