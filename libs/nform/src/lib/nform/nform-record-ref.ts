 ...
 *
 * The internal logic for handling a render record is array based (due to ordering) but the
 ...
 */
export class NFormRecordRef<T extends keyof FormElementType = keyof FormElementType> implements RenderDef<T> {
  /**
   * A object used to mark changes in the record for ngFor iterations.
   *
   * @internal
   */
  hash: any;
   ...
   *
   * If the current record is an array (has children) this represents the type of the children.
   */
  isPrimitive: boolean;

  /**
   * A virtual record is a record that is not part of the rendering flow but represents a physical control.
   * A virtual record is used for form graph control hierarchy, it is the parent of all properties of a flatten
   * expression.
   ...
   */
  virtualChildren?: NFormRecordRef[];

  /**
   * When true indicates that the record is for a FormArray.
   */
  isArray: boolean;

  /**
   * When set indicates that the record is for a FormArray.
   */
  children?: NFormRecordRef[];
   ...
   *
   * The `FormArray` instance contains `FromGroup` instances while the render record matching the `FormArray`
  ...
  constructor(public name: string, private formProp: FormPropMetadata = FormPropMetadata.EMPTY,  public parent?: NFormRecordRef) {
  ...
  /**
   * Call this function when changing properties in this [[NFormRecordRef]] instance, before calling
   * [[NFormComponent.redraw]] or when you want a specific control to re-render.
   *
