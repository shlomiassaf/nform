import { NFormControlTemplateContext, NForm } from '../nform/nform';
...
export interface RendererEventBase<T extends keyof RENDERER_EVENTS = keyof RENDERER_EVENTS> {
  type: T;
  context: NFormControlTemplateContext;
}
...
export interface ChildFormEditRendererEvent extends RendererEventBase<'childFormEdit'> {
  type: 'childFormEdit';
  isNew: boolean;
  createNForm<T extends any>(): NForm<T>;
  reset(): void;
}
...
export interface ArrayRemoveRendererEvent extends RendererEventBase<'arrayRemove'> {
  /**
